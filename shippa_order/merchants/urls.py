...
from order.admin_views import OrderListView, OrderRetrieveDestroy, OrderSalesReportView
...
urlpatterns = [
    ...
    path('<int:merchant_id>/orders/', OrderListView.as_view()),
    path('<int:merchant_id>/orders/<int:order_id>/', OrderRetrieveDestroy.as_view()),
    path('<int:merchant_id>/sales_report/', OrderSalesReportView.as_view()),
]
